...
def archive_page(page_name:str, site, archive_page_name:str = "%(page)s/存檔%(counter)d", archive_time:list = ["old", 86400], counter:int = 1, minthreadsleft:int = 5, minthreadstoarchive:int = 2, \
                    ...
                    if re.match(rule, title):
                        if custom_time_type == "old":
                            ...
                            if custom_standard[0] == "y":
                               ...
                               for j in signature_timestamp:
                                    time_then = timestripper.timestripper(j).timetuple()
                                    if (datetime.now(timezone.UTC).isocalendar()[1] - datetime(*time_then[:6]).isocalendar()[1]) < custom_standard[1]:
                                        fail = True
                                        break
                                    if calendar.timegm(time_then) > last_timestamp:
                                        last_time = time_then
            ...
            if not custom_rules_used:
                ...
                if time_type == "old":
                    ...
                    if archive_standard[0] == "y":
                       ...
                       for j in signature_timestamp:
                            time_then = timestripper.timestripper(j).timetuple()
                            if (datetime.now(timezone.UTC).isocalendar()[1] - datetime(*time_then[:6]).isocalendar()[1]) < archive_standard[1]:
                                fail = True
                                break
                            if calendar.timegm(time_then) > last_timestamp:
                                last_time = time_then
    ...
    if del_list != set() and len(del_list) >= minthreadstoarchive:
        if counter_used:
            unarchived = []
            result = archive(archive_page_name = archive_page_name, site = site, archive_list = archive_list[0], sections = sections, talk_page_name = page_name, \
            ...
            del_archived(site, talk_page, del_list, archive_list[0])
        else:
            archive(archive_page_name = archive_page_name, site = site, archive_list = archive_list[0], sections = sections, talk_page_name = page_name, \
                    header = archiveheader, counter_used = False)
